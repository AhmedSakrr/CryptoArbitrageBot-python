...
import coinbase # Shorting disabled server side until further notice
...
Limit_Qty = False  #### Limits to the minimum order qty for test purposes
Fake_MOQs = False #### Use real or fake MOQs
Fake_Balances = True #### Use real or fake balances
Fake_Prices = False   #### Use real or fake prices
Fake_Fees = False  #### Use real or fake fees
Multi_Threading = True #### Mulit-Processing runs quicker but harder to de-bug
...
if Jumpstart_Script is False:
    ...
    Coinbase = {'Name': 'Coinbase', 'Shorting': False, 'Switched-On': True}
    ...
    Wex = {'Name': 'Wex', 'Shorting': False, 'Switched-On': False} # dead
    BitBay = {'Name': 'BitBay', 'Shorting': False, 'Switched-On': True}
    Liquid = {'Name': 'Liquid', 'Shorting': True, 'Switched-On': True} # Can Short
    
    Exchanges = [Bitfinex, Kraken, Bitstamp, Coinbase, Bl3p, TheRock, Cex, Wex, BitBay, Liquid]  
...
#    Amount = '%.8f' % (amount*price)
    ...
    if side == 'buy':
        ...
        if Amount > 0:
            ...
            log.write('\nTransferring %s %s from %s wallet to %s Wallet' %(Amount, Fiat[0], 'Margin', 'Exchange'))
        ...
        if Amount > 0:
            ...
            log.write('\nTransferring %s %s from %s wallet to %s Wallet' %(Amount, Fiat[0], 'Exchange', 'Margin'))
...
def Coinbase_Private_Client():
    
    key = Keys.Coinbase()        
    client = coinbase.AuthenticatedClient(key['key'],key['secret'],key['passphrase'])
    return client
...
def Coinbase_Fees():
    ...
    fills = Coinbase_Private_Client().getFills(productId=pair)[0] # get latest filled order
    ...
    sell_fee = buy_fee #0 # fee for market makers is zero
...
def Coinbase_Limit_Order(Coin, amount, side, price): #, margin): waiting to be margin approved
    ...
    Order = Coinbase_Private_Client().order(params)

    print('Message from Coinbase: ' + str(Order))
    log.write('\nMessage from Coinbase: ' + str(Order)) 

    return Order
    
def Coinbase_Market_Order(Coin, amount, side): #, margin): waiting to be margin approved
...
def Coinbase_Check_Order(ref):    
    
    Order = Coinbase_Private_Client().getOrder(ref)
...
def Coinbase_Orderbook(Coin):
    
    pair = Coin+ '-' + Fiat[0]   
    public_client = Coinbase.PublicClient(product_id = pair)   
...
def Coinbase_Filled(Order):
    
    Order_ID = Order['ID']
    
    Filled = Coinbase_Check_Order(Order_ID)['settled']
        
    return Filled
    
def Coinbase_Cancel_Order(ref):    
    
    Order = Coinbase_Private_Client().cancelOrder(ref)
...
def Liquid_Private_Client():
    
    Key = Keys.Liquid()
    client = liquid.api.Client(Key['Key'], Key['Secret'])
    return client
    
def Liquid_Orderbook(Coin):
    
    pair = (Coin, Fiat[0])
    
    API = liquid.api.Client()
...
def Liquid_Fees():
    ...
    info = Liquid_Private_Client().get_product_info(pair)
...
def Liquid_Balances(Coin): 

    balances = Liquid_Private_Client().get_balances()
...
def Liquid_Limit_Order(Coin, amount, side, price, leverage):
    ...
    Order = Liquid_Private_Client().place_limit_order(pair, ordertype, Amount, side, Price, leverage)

    print('Message from Liquid: ' + str(Order))
    log.write('\nMessage from Liquid: ' + str(Order)) 
    
    return Order
    
def Liquid_Market_Order(Coin, amount, side, leverage):
    ...
    Order = Liquid_Private_Client().place_market_order(pair, ordertype, Amount, side, leverage)
...
def Liquid_Check_Orders(ref):
        
    Order = Liquid_Private_Client().get_order(ref)
...
def Liquid_Cancel_Order(ref):
    
    Order = Liquid_Private_Client().cancel_order(ref)
...
def Liquid_Close_Position(ref):
    
    Order = Liquid_Private_Client().close_trade(ref)
    ...
    for attempt in range(1, Attempts):     
        ...
        try:
            ...
            if Exchange['Name'] == 'Bitfinex':
                ...
                Balances = Coinbase_Balances(Coin)
            ...
            elif Exchange['Name'] == 'Liquid':
                Balances = Liquid_Balances(Coin)
...
#    return {Exchange['Name']: Balances}
    Balance_Results.update({Exchange['Name']: Balances})
    ...
    if Fake_Balances is True:
        ...
        if Multi_Threading is True:
            ...
            for i in range(len(Exchanges)):
                process = threading.Thread(target=Get_Coin_Balances, args=(Exchanges[i],))
                process.start()
            ...
            for Exchange in Exchanges:
                Exchange.update({'Balances': Balance_Results[Exchange['Name']]})
...
#                return name, fees
                Fee_Results.update({Exchange['Name']: fees})
                return None
                
                
        else:
#            return name, Dummy_Fees
            Fee_Results.update({Exchange['Name']: Dummy_Fees})
...
def Get_All_Fees():
    ...
    if Multi_Threading is False:
    ...
    elif Multi_Threading is True:
        
        threads = []
        for i in range(len(Exchanges)):
            process = threading.Thread(target=Get_Fees, args=(Exchanges[i],))
            process.start()
            threads.append(process)
        ...
        for Exchange in Exchanges:
            Exchange.update({'Fees': Fee_Results[Exchange['Name']]})
    ...
    if Fake_Prices is False:
        ...
        try:                 
            ...
            if name == 'Kraken':
                ...
                Orderbook = Coinbase_Orderbook(Coin)
            elif name == 'Bl3p':
                ...
                Orderbook = Liquid_Orderbook(Coin)    
...
#    return {Exchange['Name']: Prices} # used for multiprocessing
    Price_Results.update({Exchange['Name']: Prices})
...
def Get_All_Prices():  
    ...
    if Fake_Prices is False:
        ...
        if Multi_Threading is False:
            ...
            threads = []
            for i in range(len(Exchanges)):
                process = threading.Thread(target=Get_Coin_Prices, args=(Exchanges[i],))
                process.start()
                threads.append(process)
            ...
            for Exchange in Exchanges:
                Exchange.update({'Prices': Price_Results[Exchange['Name']]})
...
def Find_Opportunities(Permuatation):
    
    Coin = Permuatation['Coin']
    
    # sets the start values for logging of the spread histories
    
    if Permuatation['Long']['Prices'][Coin]['buy'] == 0 or \
       Permuatation['Short']['Prices'][Coin]['buy'] == 0: 
        return None
    
    Spread_In = Calc_Spread_Entry(Permuatation)
    
    Exchange1_Fees = Permuatation['Long']['Fees']['buy_fee'] + \
                     Permuatation['Long']['Fees']['sell_fee']
                     
    Exchange2_Fees = Permuatation['Short']['Fees']['buy_fee'] + \
                     Permuatation['Short']['Fees']['sell_fee']
                     
    Fees = (Exchange1_Fees + Exchange2_Fees)
    
    Enter_Target = Initial_Target_Profit*2 + Fees # only has to reach zero to close
                              
    return { 'Coin': Permuatation['Coin'],
             'Long': Permuatation['Long'],
             'Short': Permuatation['Short'], 
             'Open_Spread': Spread_In,
             'Open_Target': Enter_Target,
             'Open_Max_Spread': Spread_In,           
             'Total_Fees': Fees,
             'Target_Profit': Initial_Target_Profit}
...
def Update_Opportunities(Opportunity): 
    ...
    Exchange1_Fees = Opportunity['Long']['Fees']['buy_fee'] + \
                     Opportunity['Long']['Fees']['sell_fee']
                     
    Exchange2_Fees = Opportunity['Short']['Fees']['buy_fee'] + \
                     Opportunity['Short']['Fees']['sell_fee']
                     
    Fees = (Exchange1_Fees + Exchange2_Fees)
    
#    Av_Target_Profit = Average_Spread*2 - Fees
    Av_Target_Profit = Average_Spread - Fees

    
    if Av_Target_Profit < 0.1:
        Av_Target_Profit = 0.1
    
#    Enter_Target = Av_Target_Profit*2 + Fees # only has to reach zero to close
    Enter_Target = Target_Profit*2 + Fees
    ...
    Opportunity.update({'Open_Target': Enter_Target,
                        'Target_Profit': Av_Target_Profit,
                        'Open_Spread': Spread_In,
        ...
        for attempt in range(1, Attempts):            
            try:
                if exchange['Name'] == 'Bitfinex':
                    ...
                    Cancelled = Coinbase_Cancel_Order(ref)
                    ...
                    Cancelled = Liquid_Cancel_Order(ref)
                ...
                if Cancelled is False:
                    ...
                    print('Successfully Cancelled Order at %s ' %(exchange['Name']))
                    log.write('\nSuccessfully Cancelled Order at %s ' %(exchange['Name']))
                    return True
...
def Place_Limit_Order(Coin, Exchange, amount, side, price, Leverage):
    ...
    if Test_Mode:
        ...
        return {'ID': Order_ID, 'Position_ID': Order_ID,
                'Price': price, 'Response': 'Test Mode'}
        ...
        try:
            ...
            if Exchange['Name'] == 'Bitfinex':
                ...
                Order = Coinbase_Limit_Order(Coin, amount, side, price)
                Order_ID = str(Order['id'])
                ...
                Order = Liquid_Limit_Order(Coin, amount, side, price, Leverage)
                Order_ID = Order['id']
                try:
                    Order['trade_id']
                except:
        ...
        for attempt in range(1, Attempts):
            ...
            try:
                ...
                return {'ID': Order_ID, 'Position_ID': Position_ID,
                        'Response': Order, 'Price': Price}
                        
        else:
            return {'ID': 'n/a', 'Position_ID': 'n/a',
                        'Response': Order, 'Price': 'n/a'}
    ...
    if Test_Mode:
        ...
        try:
            if Exchange['Name'] == 'Bitfinex':        
                ...
                Order = Coinbase_Market_Order(Coin, amount, side)
                Order_ID = str(Order['id'])
                ...
                Order = Liquid_Market_Order(Coin, amount, side, leverage)
                Order_ID = Order['id']
                    
        except:
            print('Market Order at %s Failed' %(Exchange['Name']))
            log.write('\nMarket Order at %s Failed' %(Exchange['Name']))
            return False
        ...
        for attempt in range(1, Attempts):
            ...
            try:
                if Exchange['Name'] == 'Bitfinex':        
                    ...
                    if Order['status'] != 'pending':
                        Total_Value = float(Order['executed_value'])
                        Amount = float(Order['size'])
                        Price = float(Total_Value / Amount)
                    else:
                        Price = 0.0
                        raise
            ...
            if side == 'buy':
                Price = Exchanges[Exchange['Name']]['Prices'][Coin]['buy']
            elif side == 'sell':
                Price = Exchanges[Exchange['Name']]['Prices'][Coin]['sell']
...
def Close_Position(Coin, Exchange, Amount, side, leverage, ref):
    ...
    if Test_Mode:        
        ...
        if Exchange['Name'] == 'Bitfinex':        
            ...
            Order = Liquid_Close_Position(ref)
...
def Trade_Logic_Close(Coin,
    ...
    print('Sold %s %s at %s for %s %s' %(Sell_Amount, Coin, Name1, Actual_Sell_Price, Fiat[0]))
    log.write('\nSold %s %s at %s for %s %s' %(Sell_Amount, Coin, Name1, Actual_Sell_Price, Fiat[0]))
    ...
    print('Bought %s %s at %s for %s %s' %(Buy_Amount, Coin, Name2, Close_Buy_Price, Fiat[0]))
    log.write('\nBought %s %s at %s for %s %s' %(Buy_Amount, Coin, Name2, Close_Buy_Price, Fiat[0]))
    ...
    if Sell_Order is False:
        ...
        if Buy_Filled is False:
            ...
            if Abort_Order:
                ...
                print('Lost %s %s at %s' %(Loss, Fiat[0], Name2))
                log.write('\nLost %s %s at %s' %(Loss, Fiat[0], Name2))
    ...
    if Sell_Filled is False:
        ...
        for exchange in Exchanges:
            if exchange['Name'] == Name1:
                ...
                Abort_Amount = Buy_Amount * (1 - Buy_Cost*Buy_Amount)
        ...
        if Abort_Order:    
            ...
            Exchange1['Balances'][Fiat[0]] -= Loss
            
            print('Lost %s %s at %s' %(Loss, Fiat[0], Name1))
            log.write('\nLost %s %s at %s' %(Loss, Fiat[0], Name1))
...
#        
    ...
    Sell_Cost = Exchange2['Fees']['sell_fee']/100 * Trade_Amount_Sell
    
    if Name1 == 'Bl3p':
        Buy_Cost += 0.01/Buy_Price # extra 1 penny fee for bl3p        
    
    Long_Exposure = (Trade_Amount_Buy - Buy_Cost) * Buy_Price
    Short_Exposure = (Trade_Amount_Sell + Sell_Cost) * Sell_Price
    ...
    Exit_Target = Spread - Opportunity['Target_Profit']*2 - Fees # 2*profit cos has to be both sides
...
#            Exchange2_Coin = - Trade_Amount_Sell + Sell_Cost
    ...
    Current_Arbs.append({'Coin': Coin,
                         ...
                         'Target_Profit': Opportunity['Target_Profit'],
                         'Total_Fees': Fees})
...
#                    Exchange.update({'Balances': Balances})            
    ...
    Enter_Target = 100.0 #set to something impossible
...
def Save_Variables():
    ...
    Removed_Exchanges, Successful_Arbs, Profits, 
...
if __name__ == "__main__":
    ...
    if Jumpstart_Script is False:
        ...
        Initial_Target_Profit = 200.0 # Percent (after fees and slippage)
        ...
        # Cannot short ETH on Liquid        
        for Opportunity in Opportunities:
            if Opportunity['Coin'] == 'ETH' and Opportunity['Short']['Name'] == 'Liquid':
                Opportunities.remove(Opportunity)
...
#        Run_Optimisation()
        ...
        Open_Spreads = []
        for Opportunity in Opportunities:
            if Opportunity['Open_Spread'] > 0:
                Open_Spreads.append(Opportunity['Open_Spread'])
                
        if len(Open_Spreads) > 0:
            Average_Spread = sum(Open_Spreads) / len(Open_Spreads)
        else:
            Average_Spread = 0.01
