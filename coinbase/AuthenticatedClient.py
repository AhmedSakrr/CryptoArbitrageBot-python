...
from coinbase.PublicClient import PublicClient

class AuthenticatedClient(PublicClient):
    def __init__(self, key, b64secret, passphrase, api_url="https://api.pro.coinbase.com", product_id="BTC-USD"):
        self.url = api_url
        if api_url[-1] == "/":
            self.url = api_url[:-1]
    ...
    def getAccount(self, accountId):
        r = requests.get(self.url + '/accounts/' + accountId, auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()
    ...
    def getAccountHistory(self, accountId):
        list = []
        r = requests.get(self.url + '/accounts/%s/ledger' %accountId, auth=self.auth, timeout=30)
        #r.raise_for_status()
        list.append(r.json())
    ...
    def historyPagination(self, accountId, list, after):
        r = requests.get(self.url + '/accounts/%s/ledger?after=%s' %(accountId, str(after)), auth=self.auth, timeout=30)
        #r.raise_for_status()
        if r.json():
            list.append(r.json())
        if "cb-after" in r.headers:
    ...
    def getAccountHolds(self, accountId):
        list = []
        r = requests.get(self.url + '/accounts/%s/holds' %accountId, auth=self.auth, timeout=30)
        #r.raise_for_status()
        list.append(r.json())
        if "cb-after" in r.headers:
    ...
    def holdsPagination(self, accountId, list, after):
        r = requests.get(self.url + '/accounts/%s/holds?after=%s' %(accountId, str(after)), auth=self.auth, timeout=30)
        #r.raise_for_status()
        if r.json():
            list.append(r.json())
        if "cb-after" in r.headers:
    ...
    def buy(self, buyParams):
        ...
        r = requests.post(self.url + '/orders', data=json.dumps(buyParams), auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()

    def sell(self, sellParams):
        sellParams["side"] = "sell"
        r = requests.post(self.url + '/orders', data=json.dumps(sellParams), auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()

    def cancelOrder(self, orderId):
        r = requests.delete(self.url + '/orders/' + orderId, auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()
    ...
    def cancelAll(self, data=None, product=''):
        ...
        r = requests.delete(self.url + '/orders/', data=json.dumps({'product_id':product or self.productId}), auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()

    def getOrder(self, orderId):
        r = requests.get(self.url + '/orders/' + orderId, auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()

    def getOrders(self):
        list = []
        r = requests.get(self.url + '/orders/', auth=self.auth, timeout=30)
        #r.raise_for_status()
        list.append(r.json())
        if 'cb-after' in r.headers:
    ...
    def paginateOrders(self, list, after):
        r = requests.get(self.url + '/orders?after=%s' %str(after), timeout=30)
        #r.raise_for_status()
        if r.json():
            list.append(r.json())
        if 'cb-after' in r.headers:
    ...
    def getReport(self, reportId=""):
        r = requests.get(self.url + "/reports/" + reportId, auth=self.auth, timeout=30)
        #r.raise_for_status()
        return r.json()
    ...
    def order(self, Params):
        r = requests.post(self.url + '/orders', data=json.dumps(Params), auth=self.auth, timeout=30)
        return r.json()
