...
class API:
    ...
    def getheaders(self, url, secret, key):
        ...
        headers = {'User-Agent': 'API v1', 'content-type': 'application/json', 'X-TRT-KEY': key, 'X-TRT-SIGN': signature, 'X-TRT-NONCE': nonce}
        return headers
...
# BASIC API https://api.therocktrading.com/doc/v1/#api-Basic_API
    ...
    def Balance(self, fund):
        ...
        r = requests.get(url, headers = API.getheaders(url, self.secret, self.key), timeout=10)
        return r.json()
...
# MARKET API https://api.therocktrading.com/doc/v1/#api-Market_API
    ...
    def Funds(self):
        url = StdUrl+'funds'
        r = requests.get(url, headers = {'User-Agent': 'API v1', 'content-type': 'application/json'}, timeout=10)
        return r.json()
    
      # get the orderbook for a certain fund
    def OrderBook(self, fund):
        url = StdUrl+'funds/'+fund.upper()+'/orderbook'
        r = requests.get(url, headers = {'User-Agent': 'API v1', 'content-type': 'application/json'}, timeout=5)
        return r.json()
...
# TRADING API https://api.therocktrading.com/doc/v1/#api-Trading_API
    ...
    def CancelAllOrders(self, fund):
        ...
        r = requests.delete(url, headers = API.getheaders(url, self.secret, self.key), timeout=30)
        return r

      # Show the details of one order
    def CancelOrder(self, fund, orderId):
        url = StdUrl+'funds/'+fund.upper()+'/orders/'+str(orderId)
        r = requests.delete(url, headers = API.getheaders(url, self.secret, self.key), timeout=10)
        return r.json()
    ...
    def PlaceBuyOrder(self, fund, amount, price):
        ...
        r = requests.post(url, data = json.dumps(values), headers = API.getheaders(url, self.secret, self.key), timeout=30)
        return r.json()
...
# WITHDRAWAL API https://api.therocktrading.com/doc/v1/#api-Withdrawal_API
    ...
    def Withdraw(self, currency, address, amount):
        ...
        r = requests.post(url, data = json.dumps(values), headers = API.getheaders(url, self.secret, self.key), timeout=10)
        return r.json()
    ...
    # Added by GMB    
    def PlaceOrder(self, fund, amount, side, price, leverage):
        ...
        # Added by GMB    
    def PlaceMarketOrder(self, fund, amount, side, price, leverage):
