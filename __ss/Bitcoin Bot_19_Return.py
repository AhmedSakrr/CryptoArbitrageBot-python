...
Test_Balances = False #### Use real or fake balances
...
def Print_Arbitrages():
    ...
    for Arb in Current_Arbs:
        ...
        '   -   ', \
        str(round(Arb['Close_Spread'], 2)) + ' %', \
        str(round(Arb['Target_Profit_Pc'], 2)) + ' %', \
        str(round(Arb['Current_Profit_Pc'], 2)) + ' %'))
...
def Open(Opportunity):
    ...
    while True:
        ...
        Current_Arbs.append({'Coin': Coin,
                             ...
                             'Target_Profit_Pc': Target_Profit,
                             'Current_Profit_Pc': 0})
...
def Close(Arb):
    ...
    while True:
        ...
        Sell_Fee = Exchange1['Fees']['sell_fee']/100    
        Buy_Fee = Exchange2['Fees']['buy_fee']/100
        
        Long_Amount_Gross = Sell_Amount - (Sell_Amount * Sell_Fee)
        Short_Amount_Gross = Buy_Amount + (Buy_Amount * Buy_Fee)

        Long_Balance = Long_Amount_Gross * Sell_Price
        Short_Balance = 2*Short_Exposure - Short_Amount_Gross * Buy_Price
        ...
        Profit_Pc = Profit / Total_Exposure * 100
        
        Arb['Current_Profit_Pc'] = Profit_Pc
        
        # If profit not enough, break the loop, try again next time

        if Profit_Pc < Target_Profit:
            
            print '\nLooking for exit...' 
            print Coin + ' / ' + Fiat[0]
            print 'Long: ' + Exchange1['Name'] + ' / ' + \
                  'Short: '+ Exchange2['Name']+'. '+ \
                  '\nCurrent_Profit: ' + str(round(Profit_Pc,2))+'%. ' + \
                  'Target Profit: ' + str(round(Target_Profit,2)) + '%'
        ...
        # Otherwise if enough profit, place the order
        ...
        Closed_Arb = {'Coin': Coin,
                      ...
                      'Sell Fee': Sell_Fee,
                      'Buy Fee': Buy_Fee,
                      'Long_Amount_Gross': Long_Amount_Gross,
                      'Short_Amount_Gross': Short_Amount_Gross,
