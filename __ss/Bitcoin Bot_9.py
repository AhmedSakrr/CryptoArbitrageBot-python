...
Test_Balances = True #### Use real or fake balances
...
def Bitstamp_Balances(Coin):
    ...
    Coin_Balance =  float(balance['btc_balance'])
    Fiat_Balance =  float(balance['eur_balance'])
...
def Gdax_Balances(Coin):
    ...
    for account in accounts:
        if account['Currency'] == Fiat[0]:
            Coin_Balance = round(float(account['balance']),2)  
    
    for account in accounts:
        if account['currency'] == Coin:
            Fiat_Balance = round(float(account['balance']),8)
...
def Bl3p_Balances(Coin):    
    ...
    Coin_Balance =  round(float(balances['data']['wallets'][Fiat[0]]['available']['value']),2)    
    Fiat_Balance = round(float(balances['data']['wallets'][Coin]['available']['value']), 8)
...
def Wex_Balances(Coin): 
    ...
    Coin_Balance = round(float(balances[Coin]),8)
    Fiat_Balance = round(float(balances[Fiat[0]]),2)
...
def BitBay_Balances(Coin): 
    ...
    return {'BTC': Coin_Balance, 'EUR': Fiat_Balance}
...
def Update_Balances(Exchange, Coin):
    ...
    try:
        if Exchange['Name'] == 'Kraken':
            Exchange.update({'Balances': {Coin: Kraken_Balances(Coin),
                                          Fiat[0]: Kraken_Balances(Fiat[0])}})
        elif Exchange['Name'] == 'Bitstamp':
            Exchange.update({'Balances': {Coin: Bitstamp_Balances(Coin),
                                          Fiat[0]: Bitstamp_Balances(Fiat[0])}})
        elif Exchange['Name'] == 'Gdax':
            Exchange.update({'Balances': {Coin: Gdax_Balances(Coin),
                                          Fiat[0]: Gdax_Balances(Fiat[0])}})            
        elif Exchange['Name'] == 'Bl3p':
            Exchange.update({'Balances': {Coin: Bl3p_Balances(Coin),
                                          Fiat[0]: Bl3p_Balances(Fiat[0])}})            
        elif Exchange['Name'] == 'theRock':
            Exchange.update({'Balances': {Coin: theRock_Balances(Coin),
                                          Fiat[0]: theRock_Balances(Fiat[0])}})              
        elif Exchange['Name'] == 'Cex':
            Exchange.update({'Balances': {Coin: Cex_Balances(Coin),
                                          Fiat[0]: Cex_Balances(Fiat[0])}})              
        elif Exchange['Name'] == 'Wex':
            Exchange.update({'Balances': {Coin: Wex_Balances(Coin),
                                          Fiat[0]: Wex_Balances(Fiat[0])}})              
        elif Exchange['Name'] == 'BitBay':
            Exchange.update({'Balances': {Coin: BitBay_Balances(Coin),
                                          Fiat[0]: BitBay_Balances(Fiat[0])}})              
        elif Exchange['Name'] == 'Quoinex':
            Exchange.update({'Balances': {Coin: Quoinex_Balances(Coin),
                                          Fiat[0]: Quoinex_Balances(Fiat[0])}})              
    except:
...
def Get_Prices(Exchange, Coin, Result, Index):
    ...
    Result[Index] = {Coin: {'buy': Market_Ask,
                            'sell': Market_Bid,
...
def Print_Exit(Coin, Exchange1, Exchange2, spread):
    ...
    print Name1 + ' / ' + \
          Name2 + ' ' + str(round(spread, 4)) + '%'
          
    print 'Prices ' + str(Exchange1['Prices']['exit_sell']) + Fiat[0] + ' / ' + \
                      str(Exchange2['Prices']['exit_buy']) + Fiat[0]
...
def Print_Prices():
    ...
    log.write('%-14s' '%-14s' '%-12s' '%-12s' '%-12s' '%-12s' '%-12s' '%-12s' % \
          ('Instrument','Exchange', '  Ask', '  Bid', 'Entry Ask', 'Entry Bid',  'Exit Ask', 'Exit Bid'))
    ...
    for coin in Coins:
        for Exchange in Exchanges:
            try:
               ...
               (coin + ' / ' + Fiat[0], \
                Exchange['Name'], \
                Exchange[coin]['Prices']['buy'], Exchange[coin]['Prices']['sell'], \
                Exchange[coin]['Prices']['entry_buy'], Exchange[coin]['Prices']['entry_sell'], \
                Exchange[coin]['Prices']['exit_buy'], Exchange[coin]['Prices']['exit_sell'])),
            except:
                pass
            else:
                pass                   
...
def Open(Opportunity):
    ...
    while True:
        ...
        Current_Arbs.append({'Coin': Coin,
                             ...
                             'Open_Spread': Spread,
                             'Long_Exposure': Trade_Amount_Fiat,
                             'Short_Exposure': Trade_Amount_Fiat,
...
def Close(Arb):
    ...
    while True:
        ...
        Buy_Cost = Exchange2['Fees']['sell_fee']/100 * -Exchange2['Balances'][Coin]
        Sell_Cost = Exchange1['Fees']['sell_fee']/100 * Exchange1['Balances'][Coin]       
        
        Buy_Amount = round(-Exchange2['Balances'][Coin] + Buy_Cost, 8)
        Sell_Amount = round(Exchange1['Balances'][Coin] - Sell_Cost, 8)
    ...
    ###########################################################################
        ...
        if Test_Balances:
            ...
            Exchange1_Fiat = Exchange1['Balances'][Fiat[0]] + Long_Exposure + Long_Profit
            Exchange1_Coin = Exchange1['Balances'][Coin] - Sell_Amount - Sell_Cost
            Exchange2_Fiat = Exchange2['Balances'][Fiat[0]] + Short_Exposure + Short_Profit
            Exchange2_Coin = Exchange2['Balances'][Coin] + Buy_Amount - Buy_Cost
        ...
        elif Test_Balances is False:
            
            for Exchange in Arb:
                Update_Balances(Exchange, Coin)               
...
#        try:
#            Price = {}
#            Price.update(Get_Prices(Exchange, Coin))
#            Prices.update(Price)
#        except:
#            pass
#    Exchange.update({'Prices': Prices})

threads = [None] * len(Permuatations)
results = [None] * len(Permuatations)
...
for j, Exchange in enumerate(Exchanges):
    ...
    for i, Coin in enumerate(Coins):
            ...
            threads[j*len(Coins)+i] = threading.Thread(target=Get_Prices,
                                    args=(Exchange, Coin, results, j*len(Coins)+i))
            threads[j*len(Coins)+i].start()
                        
            Exchange.update({'Prices': results[j*len(Coins)+i]}) 
...
#Exchange.update({'Prices': results}) 
...
#    
#                for i in range(len(threads)):
#                    threads[i] = threading.Thread(target=Get_Prices,
#                                            args=(Exchange, Coin, results, i))
#                    threads[i].start()
#                
#                # do some other stuff
#                
