...
Startup_Script = True # Sells all the crypto back into fiat for trading
...
def Close_Position(Coin, Exchange, amount, side, leverage, ref):
    ...
    if Test_Mode:
        ...
        if Exchange['Name'] == 'Bitfinex':        
            Bitfinex_Order_ID = str(uuid.uuid4().hex) # generates 32 char fake ID
            Open_Order = {'Position': Bitfinex_Order_ID, 'Closed': True}  
    
        elif Exchange['Name'] == 'Kraken':        
            Kraken_Order_ID = str(uuid.uuid4().hex) # generates 32 char fake ID
            Open_Order = {'Position': Kraken_Order_ID, 'Closed': True}        
            
        elif Exchange['Name'] == 'Cex':            
            Cex_Order_ID = str(uuid.uuid4().hex)
            Open_Order = {'Position': Cex_Order_ID, 'Closed': True} 

        elif Exchange['Name'] == 'Quoinex':            
            Quoinex_Order_ID = str(uuid.uuid4().hex)
            Open_Order = {'Position': Quoinex_Order_ID, 'Closed': True}
        
        Position.update(Open_Order)
        ...
        for attempt in range(1, Attempts+7):
            ...
            try:
                ...
                if Exchange['Name'] == 'Bitfinex':        
                    ...
                    Filled = Check_Order_Filled(Coin, Exchange, Order, 'sell')
                    Open_Order = {'Position': Order['ID'], 'Closed': Filled, 'Order': Order}    
                    ...
                    Order_ID = str(Order['Position']['data']['id'])
                    Filled = Order['Closed']
                    Open_Order = {'Position': Order_ID, 'Closed': Filled, 'Order': Order}
                           
                elif Exchange['Name'] == 'Quoinex':            
                    Order = Quoinex_Close_Position(ref)
                    Order_ID = str(Order['Position']['id'])
                    Filled = Order['Closed']
                    Open_Order = {'Position': Order_ID, 'Closed': Filled, 'Order': Order}
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            if Exchange['Name'] == 'Kraken':
                ...
                Trade.update({'Order': {'ID': 'Kraken_Dummy_Order'}})
                return Trade
           ...
           try:
               if Order['Response']['errors']:
                   Trade.update({'Order': False})
                   return Trade
           
           except:
                Trade.update({'Order': Order})
                print Trade
                return Trade         
        ...
        return Trade # returns blank order from top of function
...
def Trade_Logic_Open(Coin,
    ...
    while True:   
        ...
        if Sell_Order['Order'] is False:
            return ({'Order': False}, {'Order': False}) # doesn't matter, nothing exceuted yet
        ...
        if Filled is False:
            ...
            return ({'Order': False}, {'Order': False})
...
def Open(Opportunity):
    ...
    while True:
        ...
        if Limit_Qty is True:
            ...
            MOQ_Buy_Price = Exchange1['Prices'][Coin]['entry_buy']
            MOQ_Sell_Price = Exchange2['Prices'][Coin]['entry_sell']
            
            Max_Fiat_Buy = (Buy_MOQ*MOQ_Buy_Price) / (1-Exchange1['Fees']['buy_fee']/100) # add so can re-sell
            Max_Fiat_Sell = (Sell_MOQ*MOQ_Sell_Price) / (1-Exchange2['Fees']['sell_fee']/100) # add so can re-sell
        ...
        if Buy_Amount >= Sell_Amount:
            ...
            if Exchange1['Fees']['currency'] == 'crypto':
                ...
                log.write('Trade Amount Buy 2: '+str(Trade_Amount_Buy))
...
#            Trade_Amount = int(Trade_Amount*100000000) / 100000000.0 # round down to 8 d.p.
             Trade_Amount_Buy = float(round(Trade_Amount_Buy, 8))
        
        Trade_Amount_Buy = float(round(Trade_Amount_Buy, 8))
        Trade_Amount_Sell = float(round(Trade_Amount_Sell, 8))
        ...
        if Buy_Order['Order'] is False or Sell_Order['Order'] is False:
            break
...
#        if Test_Mode:
        ...
        if Exchange1['Fees']['currency'] == 'fiat':
            ...
            Exchange1_Fiat = - (Trade_Amount_Buy+Buy_Cost)*Buy_Price 
            Exchange1_Coin = Trade_Amount_Buy
            
        elif Exchange1['Fees']['currency'] == 'crypto':
            
            Exchange1_Fiat = - Trade_Amount_Buy*Buy_Price 
            Exchange1_Coin = Trade_Amount_Buy - Buy_Cost
            
#        if Exchange2['Fees']['currency'] == 'fiat':
            
        # Exchange2 i.e. short position fees are always in fiat 
            
        Exchange2_Fiat = - (Trade_Amount_Sell+Sell_Cost)*Sell_Price 
        Exchange2_Coin = - Trade_Amount_Sell
...
def Close(Arb):
    ...
    Orders = {'Opened': {'buy':Arb['Buy_Order'], 'sell':Arb['Sell_Order']}}
    ...
    while True:
        ...
        if Place_Concurrent_Orders is False:
            ...
            Close_ID = Arb['Sell_Order']['Order']['Position_ID']
