    ...
    if spread > Entry_Spread:
        ...
        if Arb_Exists is False:
            ...
            if Exchange1['EUR'] > Min_Bal and Exchange2['EUR'] > Min_Bal:
                ...
                ###### Get Order Book for exchanges, only if luquididty execute orders
                
                Buy_Order = Place_Order(Exchange1, Trade_Amount_Buy, 'buy', Buy_Price)
...
def Update(Arb):
    ...
    Exchanges = Update_Prices(Arb[2])

    spread = Calc_Spread((Exchanges[0], Exchanges[1]))
    
    Arb[0].update(Exchanges[0])
    Arb[1].update(Exchanges[1])                         
...
def Close(Arb):    
    ...
    if spread < Exit_Spread:        
        ...
        print Exchange1['Name']+' / '+Exchange2['Name']+'Profit = ' +str(Profit) + 'EUR\n'+ \
                            'Start Time: '+ str(Arb[2]['open_time'])+ ' End Time: '+ str(datetime.now()) + '\n'\
