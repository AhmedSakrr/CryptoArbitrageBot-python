...
Bitstamp = {'Name': 'Bitstamp', 'Shorting': True}
Gdax = {'Name': 'Gdax', 'Shorting': True}
Bl3p = {'Name': 'Bl3p', 'Shorting': True}
TheRock = {'Name': 'theRock', 'Shorting': True} ##
Gatecoin = {'Name': 'Gatecoin', 'Shorting': True}
...
'''
Include_Fees = True # Used for testing to lower spread
...
def Find_Opportunities(Opportunity):
    ...
    if Include_Fees is False:
        Fees = 0
    
    Enter = Target_Profit/2 + Fees/2 + 2*Max_Slippage
    
    Exit = -Target_Profit/2 - Fees/2 - 2*Max_Slippage
       
    
    Opportunities.append([Exchange1, Exchange2,
                          Spread, Enter, Exit, Min_Spread, Max_Spread])
                          
                          
def Update_Opportunities(Opportunity):    
    
    Min_Spread = Opportunity[5]
    Max_Spread = Opportunity[6]
    ...
    Opportunity[5] = Min_Spread
    Opportunity[6] = Max_Spread        
...
def Print_Main_Info():
    ...
    print '%-22s' '%-10s' '%-10s' '%-10s' '%-10s' '%s' % \
          ('Exchange Pairs', 'Spread', 'Entry', ' Exit', '  Min', '  Max')
...
def Open(Opportunity):
    ...
    while True:
        ...
        if Liquid_Price < Min_Price:
            ...
            print 'Liquid Price: %s < Order Price: %s'%(Liquid_Price,Min_Price)
        ...
        if Test_Mode:            
            ...
            Buy_EUR = Balances[Exchange1['Name']]['EUR']-Buy_Amount*Buy_Price
            Buy_BTC = Balances[Exchange1['Name']]['BTC']+Buy_Amount
            Sell_EUR = Balances[Exchange2['Name']]['EUR']-Sell_Amount*Sell_Price
...
def Close(Arb):    
    ...
    ###########################################################################
        ...
        if Test_Mode:
            ...
            Buy_EUR = Balances[Name1]['EUR'] + Sell_Amount*Sell_Price
            Buy_BTC = Balances[Name1]['BTC'] - Sell_Amount
            Sell_EUR = Balances[Name2]['EUR'] + Buy_Amount*Buy_Price
            Sell_BTC = Balances[Name2]['BTC'] + Buy_Amount
