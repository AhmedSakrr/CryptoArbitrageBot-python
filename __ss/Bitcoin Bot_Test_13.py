...
Exit_Spread = -Target_Profit/2 # Percent to exit on after fees
Entry_Spread = Target_Profit/2 + 4*Max_Slippage # 4 transactions total, impacts profit
...
def Find_Opportunities(Opportunity):
    ...
    Opportunities.append([Exchange1, Exchange2,
                          Spread, Target, Min_Spread, Max_Spread])
...
def Close(Arb):    
    ...
    ###########################################################################
        ...
        if Test_Mode:
            ...
            Balances[Name1].update({'EUR': Buy_EUR})
            Balances[Name1].update({'BTC': Buy_BTC})
            Balances[Name2].update({'EUR': Sell_EUR})
            Balances[Name2].update({'BTC': Sell_BTC})            
...
while True:
    ...
    print '%-22s' '%-10s' '%-10s' '%-10s' '%s' % \
          ('Exchanges', 'Spread', 'Target', 'Min', 'Max')
