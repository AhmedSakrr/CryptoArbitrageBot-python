...
Limit_Arbitrages = True ## Limits the number of arbitrages to one for testing.

iteration = 1 # starts the main loop counter, only used for fake prices


#Kraken = {'Name': 'Kraken', 'Shorting': True, 'Switched-On': True} # Not reliable enough
...
TheRock = {'Name': 'theRock', 'Shorting': False, 'Switched-On': True} # No naked shorts :(
...
def theRock_Market_Order(Coin, amount, side):
    ...
    if side == 'buy':
            price = TheRock[Coin]['entry_buy'] * 1+0.00005 # beats market price
    elif side == 'sell':
            price = TheRock[Coin]['entry_sell'] * 1-0.00005 # beats market price
...
def Cex_Abort_Position(Coin, ref):
...
def Quoinex_Abort_Position(ref):
...
def Check_Order_Aborted(Exchange, Order):
    ...
    for attempt in range(1, Attempts + 7):     
        ...
        try:
            ...
            print 'Aborted order at %s waiting to fill on attempt: %i' %(name,
                                                                       attempt)
            log.write('\nAborted order at %s waiting to fill on attempt: %i'%(name,
                                                                           attempt))
        ...
        print 'Aborted order at %s failed to fill...' %Exchange['Name']
        log.write('\nAborted order at %s failed to fill...' %Exchange['Name'])
        return False               
...
def Place_Market_Order(Coin, Exchange, amount, side, leverage):    
    ...
    if Test_Mode:
        ...
        if Exchange['Name'] == 'Kraken':        
            ...
            if leverage == 0:
                Cex_Open_Order = Cex_Market_Order(Coin, amount, side, price)
                Cex_Order_ID = str(Cex_Open_Order['id'])
                Open_Order = {'ID': Cex_Order_ID, 'Order': Cex_Open_Order}
...
def Abort_Short_Position(Coin, Exchange, amount, side, leverage, ref):
        ...
        for attempt in range(1, Attempts+7):
            ...
            try:
                if Exchange['Name'] == 'Kraken':        
                    Kraken_Open_Order = Kraken_Market_Order(Coin, amount, side, leverage)
                    Kraken_Order_ID =  str(Kraken_Open_Order['result']['txid'][0])
                    Open_Order = {'ID': Kraken_Order_ID, 'Cancelled': Kraken_Open_Order}    
                    
                elif Exchange['Name'] == 'Cex':
                    Cex_Open_Order = Cex_Abort_Position(Coin, ref)
                    Cex_Order_ID = str(Cex_Open_Order['data']['id'])
                    Open_Order = {'ID': Cex_Order_ID, 'Cancelled': Cex_Open_Order}
                           
                elif Exchange['Name'] == 'Quoinex':            
                    Quoinex_Open_Order = Quoinex_Abort_Position(ref)
                    Quoinex_Order_ID = Quoinex_Open_Order['id']
                    Open_Order = {'ID': Quoinex_Order_ID, 'Cancelled': Quoinex_Open_Order}
                                
            except:
                
                print 'Abort Position Failed at %s on Attempt: %i' %(name, 
                                                                   attempt)
                log.write('\nAbort Position Failed at %s on Attempt: %i' %(name, 
                                                                         attempt))
                ...
                if Open_Order['Cancelled'] is False:
                    print 'Abort Position Failed at %s on Attempt: %i' %(name, 
                                                                       attempt)
                    log.write('\nAbort Position Failed at %s on Attempt: %i' %(name, 
                                                                             attempt))
            ...
            print 'Failed to Abort Position at %s ' %(exchange['Name'])
            log.write('\nFailed to Abort Position at %s ' %(exchange['Name']))
            return False        

def Close_Short_Position(Coin, Exchange, amount, side, leverage, ref):
    ...
    if Test_Mode:
        ...
        for attempt in range(1, Attempts+7):
            ...
            try:
                if Exchange['Name'] == 'Kraken':        
                    Kraken_Open_Order = Kraken_Settle_Position(Coin, amount, side, leverage)
                    Kraken_Order_ID =  str(Kraken_Open_Order['result']['txid'][0])
                    Open_Order = {'ID': Kraken_Order_ID, 'Cancelled': Kraken_Open_Order}    
        ...
        if Type == "Open":
            ...
            if Filled is False:
                ...
                while True:
                    ...
                    Abort_ref = Sell_Order['ID']
                                                    
                    Abort_Order = Abort_Short_Position(Coin, Exchange2, Sell_Amount, 'buy', Short_Leverage, Abort_ref)
                    
                    Filled = Check_Order_Aborted(Exchange2, Abort_Order)
        ...
        if Sell_Order['Placed'] is False:
            ...
            if Filled is False:
                ...
                while True:
                    ...
                    Abort_ref = Buy_Order['ID']
                                                
                    Abort_Order = Abort_Short_Position(Coin, Exchange1, Buy_Amount, 'sell', Long_Leverage, Abort_ref)
                
                    Filled = Check_Order_Aborted(Exchange1, Abort_Order)
...
def Open(Opportunity):
    ...
    while True:
        ...
        if Limit_MOQ is True:
            ...
            Trade_Amount_Fiat = MOQ*MOQ_Price
        ...
        if Place_Concurrent_Orders is False:
            ...
            Buy_Order, Sell_Order = Trade_Logic_Simple(Coin, Exchange1, Exchange2,
                                                       Buy_Price, Sell_Price,
                                                       Buy_Amount, Sell_Amount,
                                                       'Open')
...
def Close(Arb):
    ...
    while True:
        ...
        if Place_Concurrent_Orders is False:
            ...
            Buy_Order, Sell_Order = Trade_Logic_Simple(Coin, Exchange2, Exchange1,
                                                       Buy_Price, Sell_Price, 
                                                       Buy_Amount, Sell_Amount,
                                                       'Close')
...
if __name__ == "__main__":
    ...
    Exchanges = [Bitstamp, Gdax, Bl3p, TheRock, Cex, Wex, BitBay, Quoinex]  
