...
def isliquid(exchange, amount, side):
    ...
    if side == 'buy':
        ...
        if exchange['Name'] == 'Kraken':
            Liquid_Price = Weighted_Price(amount, Kraken_Orderbook()[0])
            
        elif exchange['Name'] == 'Bitstamp':
            Liquid_Price = Weighted_Price(amount, Bitstamp_Orderbook()[0])
            
        elif exchange['Name'] == 'Gdax':
            Liquid_Price = Weighted_Price(amount, Gdax_Orderbook()[0])
            
        elif exchange['Name'] == 'Bl3p':
            Liquid_Price = Weighted_Price(amount, Bl3p_Orderbook()[0])
            
        elif exchange['Name'] == 'theRock':
            Liquid_Price = Weighted_Price(amount, theRock_Orderbook()[0])
            
        elif exchange['Name'] == 'Gatecoin':
            Liquid_Price = Weighted_Price(Gatecoin, Kraken_Orderbook()[0])
            
    elif side == 'sell':
    
        if exchange['Name'] == 'Kraken':
            Liquid_Price = Weighted_Price(amount, Kraken_Orderbook()[1])
            
        elif exchange['Name'] == 'Bitstamp':
            Liquid_Price = Weighted_Price(amount, Bitstamp_Orderbook()[1])
            
        elif exchange['Name'] == 'Gdax':
            Liquid_Price = Weighted_Price(amount, Gdax_Orderbook()[1])
            
        elif exchange['Name'] == 'Bl3p':
            Liquid_Price = Weighted_Price(amount, Bl3p_Orderbook()[1])
            
        elif exchange['Name'] == 'theRock':
            Liquid_Price = Weighted_Price(amount, theRock_Orderbook()[1])
            
        elif exchange['Name'] == 'Gatecoin':
            Liquid_Price = Weighted_Price(Gatecoin, Kraken_Orderbook()[1])
...
def Execute_Order(Exchange, Trade_Amount, Order_Price, side, slippage):  
    ...
    ''' Get Order Book for exchanges to determine liquidity, only if liquidity execute orders''' 
    if side == 'buy':
    ...
    if Buy_Liquid_Price < Buy_Order_Price or Sell_Liquid_Price > Sell_Order_Price:
        
        for i in range(1,10):
            while True:                    
                try:
                    Order = Place_Order(Exchange, Trade_Amount, side, Order_Price) # Places the order
                    print side+' Order Placed on attempt: %i' %i
                    log_file.write(side+' Order Placed on attempt: %i' %i)
                    Placed = True
                except:
                    print side+' Order failed on attempt: %i' %i
                    log_file.write(side+' Order failed on attempt: %i' %i)
                    continue
                
                Order = False
                print side+' Order Failed to place, aborting!!!'
                break
    ...
    for i in range(10):                        
        while True:                    
            try:                
                ...
                print side+' Order Executed on attempt: %i' %i
                log_file.write(side+' Executed on attempt: %i' %i)
                
                if side == 'buy':
                    print 'Bought ' + str(Trade_Amount) + pair1[0] + ' at ' + str(Exchange['Name'])        
                    log_file.write('Bought ' + str(Trade_Amount) + pair1[0] + ' at ' + str(Exchange['Name']))                    
                    
                elif side  == 'sell':
                    print 'Sold ' + str(Trade_Amount) + pair1[0] + ' at ' + str(Exchange['Name'])        
                    log_file.write('Sold ' + str(Trade_Amount) + pair1[0] + ' at ' + str(Exchange['Name']))
                
                time.sleep(3)           
                
            except:
                print side+' Order waiting to fill...'
                log_file.write(side+' Order waiting to fill...')
                continue        
                                        
            print side+' Order failed to fill, aborting!!!'
            log_file.write(side+' Order failed to fill, aborting!!!')
            Order_Filled = False
            break
...
def Print_Profit(Exchange1, Exchange2, Profit):
    ...
    if Spread > Entry_Spread:
        ...
        if Arb_Exists is False:
            ...
            if Exchange1['EUR'] > Min_Bal and Exchange2['EUR'] > Min_Bal:
                ...
                Buy_Order = Execute_Order(Exchange1, Trade_Amount_Buy, Buy_Price, 'buy', slippage)                
                if Buy_Order['Placed'] is True:                    
                    Sell_Order = Execute_Order(Exchange2, Trade_Amount_Sell, Sell_Price, 'sell', slippage)
                    if Sell_Order['Placed'] is True:
                        Check_Buy_Filled = Check_Order_Filled(Exchange1, Buy_Order['Order'], Trade_Amount_Buy, 'buy')
                        if Check_Buy_Filled is True:
                            Check_Sell_Filled = Check_Order_Filled(Exchange2, Sell_Order['Order'], Trade_Amount_Sell, 'sell')                            
                ...
                log_file.write('Insufficient Funds for ' + Exchange1['Name']+' / '+Exchange2['Name']+' '+str(round(Spread,2))+'%')
...
def Close(Arb):    
    ...
    if spread < Exit_Spread:        
        ...
        ''' If one of the orders doesn't close, move to failed orders and try to re-enter opposite order to remain market neutral ''' 
        
        Sell_Order = Execute_Order(Exchange1, Sell_Amount, Sell_Price, 'sell', slippage)        
        Buy_Order = Execute_Order(Exchange1, Buy_Amount, Buy_Price, 'buy', slippage) # Need to look at closing the short by order ID if possible.
        
        Update((Exchange1,Exchange2))                
        ...
        Print_Profit(Exchange1, Exchange2, Profit)       
