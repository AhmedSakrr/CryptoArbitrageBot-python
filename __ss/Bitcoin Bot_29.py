...
def Place_Limit_Order(Coin, Exchange, amount, side, price, Leverage):
    ...
    if Test_Mode:
        ...
        try:
            ...
            return {'ID': 'N/A', 'Position_ID': 'N/A', 'Response': False}
...
def Place_Market_Order(Coin, Exchange, amount, side, leverage):
    ...
    #            Price = float(Order['price'])
                ...
                if leverage == 0:
                    ...
                    Order_ID = str(Order['id'])
                    Price = float(Order['symbol2Amount'])/100.0
                elif leverage != 0:
                    Order = Cex_Open_Position(Coin, amount, side, price, leverage)
                    Order_ID = str(Order['data']['id'])
                    Price = price
            ...
            return {'ID': 'N/A', 'Price': 0.0, 'Response': False} 
...
def Trade_Logic_Close(Coin,
    ...
    for attempt in range(1, Attempts):
        ...
        print 'Checking if Sell Order at %s filled. Attempt: %i' %(Name1, attempt)
        log.write('\nChecking if Sell Order at %s filled on attempt: %i'%(Name1, attempt))
            
        Sell_Filled = Check_Order_Filled(Coin, Exchange1, Sell_Order, 'sell')
        
        if Sell_Filled is False:
            print 'Sell Order at %s did not fill on attempt: %i' %(Name1, attempt)
            log.write('\nSell Order at %s did not fill on attempt: %i'%(Name1, attempt))
            ...
            print 'Sell Order at %s filled on attempt: %i' %(Name1, attempt)
            log.write('\nSell Order at %s filled on attempt: %i'%(Name1, attempt))
            break
    ...
    print 'Sold '+str(Sell_Amount)+Coin+' at '+Name1+' for '+str(Sell_Price)
    log.write('\nSold '+str(Sell_Amount)+Coin+' at '+Name1+' for '+str(Sell_Price))
    ...
    print 'Bought '+str(Buy_Amount)+Coin+' at '+Name2+' for '+str(Close_Buy_Price)
    log.write('\nBought '+str(Buy_Amount)+Coin+' at '+Name2+' for ' + str(Close_Buy_Price))
...
def Trade_Logic_Complex_Open(Coin,
    ...
    log.write('\nBought %s %s at %s for %s %s' %(Buy_Amount, Coin, Name2, Buy_Price, Fiat[0]))
...
#        log.write('\nBuy_Amount: ' + str(Buy_Amount))       
    ...
    if Close_Buy_Price > Buy_Price:
        ...
        log.write('Suffered from %s Percent Slippage on Buy Order' %Slippage)
        
    if Close_Buy_Price < Buy_Price:
        Slippage = (Buy_Price - Close_Buy_Price) / Buy_Price * 100
        print 'Benefited from %s Percent Slippage on Buy Order' %Slippage
        log.write('Benefited from %s Percent Slippage on Buy Order' %Slippage)
        
    Buy_Price = Close_Buy_Price
