...
Limit_MOQ = True  #### Limits to the minimum order qty for test purposes
...
Place_Concurrent_Orders = False ## Simple executes both trades one after another
...
Kraken = {'Name': 'Kraken', 'Shorting': False, 'Switched-On': True} # advanced order currently disabled 20/01/18
Bitstamp = {'Name': 'Bitstamp', 'Shorting': False, 'Switched-On': True}
Gdax = {'Name': 'Gdax', 'Shorting': False, 'Switched-On': True}
Bl3p = {'Name': 'Bl3p', 'Shorting': False, 'Switched-On': True}
TheRock = {'Name': 'theRock', 'Shorting': False, 'Switched-On': True}
Cex = {'Name': 'Cex', 'Shorting': False, 'Switched-On': True} # waiting for new moqs 23/01/18
...
def Cex_Filled(Coin, Order):
    ...
    try:    
        Order_ID = Order['Order']['id'] # only works for non margin order
    except:
        try:
            Order_ID = Order['Order']['data']['id'] # checks margin order
        except:
            return False
...
def Cex_Close_Position(Coin, ref):
    ...
    return {'Position': Order, 'Closed': Closed}  
...
def Quoinex_Filled(Order):
    ...
    filled_quantity = Order['Order']['filled_quantity']
...
def Quoinex_Close_Position(ref):
    ...
    Closed_Trade = Quoinex_Private_Client().close_trade(ref)
    
    print 'Message from Quoinex: ' + str(Closed_Trade)
    log.write('\nMessage from Quoinex: ' + str(Closed_Trade))
    
    if Closed_Trade['status'] == 'closed':
        filled = True
    else:
        filled = False
        
    return {'Position': Closed_Trade, 'Closed': filled}
...
def Get_Fees(Exchange):
    ...
    if Fake_Prices is False:
        ...
        for attempt in range(1, Attempts+3):     
            ...
            try:
                if name == 'Bitfinex':
                    Exchange.update({'Fees': Bitfinex_Fees()})
                elif name == 'Kraken':
...
def Check_Position_Closed(Exchange, Order):
    ...
    for attempt in range(1, Attempts + 7):     
        ...
        try:
            ...
            print 'Closing Position at %s waiting to fill on attempt: %i' %(name,
                                                                       attempt)
            log.write('\nClosing Position at %s waiting to fill on attempt: %i'%(name,
                                                                           attempt))
        ...
        print 'Closing Position at %s failed to fill...' %Exchange['Name']
        log.write('\nClosing Position at %s failed to fill...' %Exchange['Name'])
        return False               
...
def Place_Limit_Order(Coin, Exchange, amount, side, price, Leverage):
    ...
    if Test_Mode:
        ...
        if Exchange['Name'] == 'Bitfinex':
            ...
            Open_Order = {'ID': Quoinex_Order_ID, 'Position_ID': Quoinex_Position_ID, 'Order': Quoinex_Open_Order}
...
def Place_Market_Order(Coin, Exchange, amount, side, leverage):    
    ...
    if Test_Mode:
        ...
        if Exchange['Name'] == 'Bitfinex':        
            ...
            Bitfinex_Order_ID =  str(Bitfinex_Open_Order['order_id'])
            Open_Order = {'ID': Bitfinex_Order_ID, 'Order': Bitfinex_Open_Order}    

        elif Exchange['Name'] == 'Kraken':        
            Kraken_Open_Order = Kraken_Market_Order(Coin, amount, side, leverage)
            Kraken_Order_ID =  str(Kraken_Open_Order['result']['txid'][0])
            Open_Order = {'ID': Kraken_Order_ID, 'Order': Kraken_Open_Order}    
            ...
            Quoinex_Order_ID = Quoinex_Open_Order['id']
            Open_Order = {'ID': Quoinex_Order_ID, 'Order': Quoinex_Open_Order}
...
def Close_Position(Coin, Exchange, amount, side, leverage, ref):
    ...
    if side == 'buy':
        side = 'sell' # Sounds stupid, but Kraken shorts are settled with sell
    ...
    if Test_Mode:
        ...
        for attempt in range(1, Attempts+7):
            ...
            try:
                ...
                if Exchange['Name'] == 'Bitfinex':        
                    Bitfinex_Open_Order = Bitfinex_Market_Order(Coin, amount, side, leverage)
                    Bitfinex_Order_ID =  str(Bitfinex_Open_Order['result']['txid'][0])
                    Filled = Check_Order_Filled(Coin, Exchange, Bitfinex_Open_Order, 'sell')
                    Open_Order = {'Position': Bitfinex_Order_ID, 'Closed': Bitfinex_Open_Order}    
                
                elif Exchange['Name'] == 'Kraken':        
                    Kraken_Open_Order = Kraken_Market_Order(Coin, amount, side, leverage)
                    Kraken_Order_ID =  str(Kraken_Open_Order['result']['txid'][0])
                    Filled = Check_Order_Filled(Coin, Exchange, Kraken_Open_Order, 'sell')
                    Open_Order = {'Position': Kraken_Order_ID, 'Closed': Filled}    
                    
                elif Exchange['Name'] == 'Cex':
                    Cex_Open_Order = Cex_Close_Position(Coin, ref)
                    Cex_Order_ID = str(Cex_Open_Order['Position']['data']['id'])
                    Open_Order = {'Position': Cex_Order_ID, 'Closed': Cex_Open_Order['Closed']}
                           
                elif Exchange['Name'] == 'Quoinex':            
                    Quoinex_Open_Order = Quoinex_Close_Position(ref)
                    Quoinex_Order_ID = str(Quoinex_Open_Order['Position']['id'])
                    Open_Order = {'Position': Quoinex_Order_ID, 'Closed': Quoinex_Open_Order['Closed']}
...
def Execute_Limit_Order(Coin, Exchange, Amount, Side, Price, Leverage):
    ...
    # warning multiple attempts on Kraken will fuck you up!!
    # Exchange actually executes order but doesn't return shit to api!!
    ...
    for attempt in range(1, attempts + 1): 
        ...
        try:
            ...
            # Kraken is a piece of shit, just assume order was placed, check later.
            if Exchange['Name'] == 'Kraken':
                
                print 'Message: ' + [i for i in traceback.format_exc().split('\n') if i][-1]
                log.write('Message: ' + [i for i in traceback.format_exc().split('\n') if i][-1])
                
                print 'Kraken Dummy Order Placed'
                log.write('Kraken Dummy Order Placed')            
                return {'Order': {'ID': 'Kraken_Dummy_Order'}, 'Placed': True}                
                                                                      
            print 'Message: ' + [i for i in traceback.format_exc().split('\n') if i][-1]
            log.write('Message: ' + [i for i in traceback.format_exc().split('\n') if i][-1])
            
            print Side + ' Order failed at %s on attempt: %s' %(name, attempt)
            log.write('\n' + Side + ' Order failed at %s on attempt: %s' %(name, attempt)) 
        ...
        print Side + ' Limit Order failed at %s on all attempts' %(name)
        log.write('\n' + Side + ' Limit Order failed at %s on all attempts' %(name))
...
def Execute_Market_Order(Coin, Exchange, Amount, Side, Leverage):
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            print Side + ' Market Order failed at %s on attempt: %s' %(name, attempt)
            log.write('\n' + Side + ' Market Order failed at %s on attempt: %s' %(name, attempt))
...
def Trade_Logic_Open(Coin,
    ...
    if Exchange2['Name'] == 'Bitfinex':
        Sell_Amount = Sell_Amount * 1.0
    elif Exchange2['Name'] == 'Kraken':
        Sell_Amount = Sell_Amount * 1.0
    elif Exchange2['Name'] == 'CEX':
        Sell_Amount = Sell_Amount * 1.0
    elif Exchange2['Name'] == 'Quoinex':
        Sell_Amount = Sell_Amount * 1.0
    ...
    while True:   
        ...
        Filled = Check_Order_Filled(Coin, Exchange2, Sell_Order['Order'], 'sell') 
        
        if Filled is False:
        
            Cancel_Order(Coin, Exchange2, Sell_Order['Order']['ID'])
            
            print 'Coin: ' + str(Coin) + \
                  ' Exchange: '+ str(Exchange2['Name']) + \
                  ' Sell Order ID: '+ str(Sell_Order['Order']['ID']) + \
                  ' Cancelled'
        ...
        Filled = Check_Order_Filled(Coin, Exchange1,Buy_Order['Order'], 'buy')
    
        if Filled is False:
            
            Cancel_Order(Coin, Exchange1, Buy_Order['Order']['ID'])
            
            print 'Coin: ' + str(Coin) + \
                  ' Exchange: ' + str(Exchange1['Name']) + \
                  ' Buy Order ID: '+ str(Buy_Order['Order']['ID']) + \
                  ' Cancelled'
            ...
            while True:
                ...
                Abort_ref = Sell_Order['Order']['Position_ID']
                                                
                Abort_Order = Close_Position(Coin, Exchange2, Sell_Amount, 'buy', Short_Leverage, Abort_ref)
                
                Filled = Check_Position_Closed(Exchange2, Abort_Order)
            ...
            print 'Coin: ' + str(Coin) + \
                  ' Exchange: ' + str(Exchange2['Name']) + \
                  ' Order ID: '+ str(Abort_Order['Order']['ID']) + \
                  ' Trade Reversed'
...
def Trade_Logic_Close(Coin,
    ...
    while True:   
        ...
        Sell_Order = Execute_Limit_Order(Coin, Exchange1, Sell_Amount, 'sell', Sell_Price, Short_Leverage)                                  
                 
        if Sell_Order['Placed'] is False:
            return ({'Placed': False},{'Placed': False}) # doesn't matter, nothing exceuted yet
        
        Filled = Check_Order_Filled(Coin, Exchange1, Sell_Order['Order'], 'sell') 
        
        if Filled is False:
        
            Cancel_Order(Coin, Exchange1, Sell_Order['Order']['ID'])
        ...
        print 'Sold '+str(Sell_Amount)+Coin+' at '+Exchange1['Name'] + \
              ' @ ' + str(Sell_Price) + ' @ '+str(Short_Leverage)+'x Leverage'
              
        log.write('\nSold '+str(Sell_Amount)+Coin+' at '+Exchange1['Name']+\
                  ' @ ' + str(Sell_Price) + ' @ '+str(Short_Leverage)+'x Leverage')
        ...
        while True:
            ...
            Closed_Position = Close_Position(Coin, Exchange2, Buy_Amount, 'buy', Long_Leverage, Close_ID)
            
            Filled = Check_Position_Closed(Exchange2, Closed_Position)
            ...
            print 'Bought '+str(Buy_Amount)+Coin+' at '+Exchange2['Name'] + \
                  ' @ ' + str(Buy_Price) + ' @ '+str(Long_Leverage)+'x Leverage'
                  
            log.write('\nBought '+str(Buy_Amount)+Coin+' at '+Exchange2['Name']+\
                      ' @ ' + str(Buy_Price) + ' @ '+str(Long_Leverage)+'x Leverage')         
...
def Trade_Logic_Complex(Coin,
    ...
    if Type == 'Open':
        ...
        Sell_Amount = Sell_Amount / 2.0
        
    elif Type == 'Close':
        Long_Leverage = 2
        Short_Leverage = 0
        
        Buy_Amount = Buy_Amount / 2.0
        ...
        if Sell_Order['Placed'] is False:
            ...
            if Filled is False:
                ...
                while True:
                    ...
                    Abort_ref = Buy_Order['Order']['Position_ID']
                                                
                    Abort_Order = Close_Position(Coin, Exchange1, Buy_Amount, 'sell', Long_Leverage, Abort_ref)
                
                    Filled = Check_Position_Closed(Exchange1, Abort_Order)
...
def Open(Opportunity):
    ...
    Buy_Order = {'Placed': False}
    Sell_Order = {'Closed': False}
    ...
    while True:
        ...
        if Limit_MOQ is True:
            ...
            MOQ_Price = Exchange2['Prices'][Coin]['sell']
            
            Trade_Amount_Fiat = MOQ*MOQ_Price * 1.2 # add 20% so can re-sell
            
            if Trade_Amount_Fiat > Balance1*0.995 or \
               Trade_Amount_Fiat > Balance2*0.995:
                break
            
        elif Limit_MOQ is False:
        ...
        # Use exchange with minimum balance to execute orders
            ...
            if Balance1 > Balance2:
                ...
                Trade_Amount_Fiat = Balance1 * 0.995
        ...
        if Name1 == 'theRock':
            ...
            if Coin == 'LTC':
                Buy_Amount = int(Buy_Amount*100) / 100.0 # round down to 2 d.p.
                Sell_Amount = int(Sell_Amount*100) / 100.0 # round down to 2 d.p.
            if Coin == 'ETH':
                Buy_Amount = int(Buy_Amount*100) / 100.0 # round down to 2 d.p.
                Sell_Amount = int(Sell_Amount*100) / 100.0 # round down to 2 d.p.

        else:
            Buy_Amount = int(Buy_Amount*100000) / 100000.0 # round down to 5 d.p.
            Sell_Amount = int(Sell_Amount*100000) / 100000.0 # round down to 5 d.p.
        ...
        if Place_Concurrent_Orders is False:
            ...
            Buy_Order, Sell_Order = Trade_Logic_Complex(Coin, Exchange1, Exchange2,
                                                         Buy_Price, Sell_Price,
                                                         Buy_Amount, Sell_Amount,
                                                         'Open')
...
def Close(Arb):
    ...
    Buy_Order = {'Closed': False}
    Sell_Order = {'Placed': False}
    ...
    while True:
        ...
        if Place_Concurrent_Orders is False:
            ...
            Close_ID = Arb['sell_id']['Position_ID']
            ...
            Buy_Order, Sell_Order = Trade_Logic_Complex(Coin, Exchange2, Exchange1,
                                                        Buy_Price, Sell_Price, 
                                                        Buy_Amount, Sell_Amount,
                                                        'Close')
                                                                                            
        if Buy_Order['Closed'] is False or Sell_Order['Placed'] is False:
            break
        
        Orders.update({'Closed': {'buy':Buy_Order['Position'], 'sell':Sell_Order['Order']}})
...
#            Open(Opportunity)
        ...
        Open(Opportunities[0]) # only opens first, next opp could slip
