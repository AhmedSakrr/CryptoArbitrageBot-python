...
def Print_Exit(Coin, Exchange1, Exchange2, spread):
    ...
    log.write('\nPrices ' + str(Exchange1['Prices']['exit_sell']) + Fiat[0] + ' / ' + \
                      str(Exchange2['Prices']['exit_buy']) + Fiat[0])
...
def Get_All_Prices():
    ...
    for Exchange in Exchanges:
        for i in result.get():
            if i.keys()[0] == Exchange['Name']:
                Exchange.update({'Prices': i.values()[0]})
                
#    pool.close()
#    pool.join()


def calculate_spread(Spreads):
    Spread_Trace = []
    pprint.pprint(Spreads)
    for Opportunity in Opportunities:
        for C in Coins:
            history = []
            for item in Spreads[0]:
                coin = item[0]
                pair = item[1]
                spread = item[2]
                
                if Opportunity['Long']['Name'] + '_' + Opportunity['Short']['Name'] in pair and \
                   C in coin:
                   
                   history.append(spread)
            Spread_Trace.append((coin, pair, history))
    pprint.pprint(Spread_Trace) 
    return Spread_Trace
...
if __name__ == "__main__":
    ...
    while True:
        ...
        sprd = []
        for Opportunity in Opportunities:
            pair = Opportunity['Long']['Name'] + '_' + Opportunity['Short']['Name']            
            sprd.append(( Opportunity['Coin'], pair, Opportunity['Open_Spread']))
        ...
        now = datetime.now()
        if i == 5:
            new_spread = calculate_spread(Spreads)
            i = 0
            #update_spread(new_spread)
    
        
#        
#            
#            = {'Price' : []}
#            Statistics['Name'] = {Exchange['Name']: {'Prices': []}}
#            Statistics[Exchange['Name']]['Prices'].append(Exchange['Prices'])
