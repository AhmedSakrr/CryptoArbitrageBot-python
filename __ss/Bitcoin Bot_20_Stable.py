...
Kraken = {'Name': 'Kraken', 'Shorting': True, 'Switched-On': True} ##
...
Cex = {'Name': 'Cex', 'Shorting': True, 'Switched-On': False}
Wex = {'Name': 'Wex', 'Shorting': False, 'Switched-On': True}
BitBay = {'Name': 'BitBay', 'Shorting': False, 'Switched-On': False}
Quoinex = {'Name': 'Quoinex', 'Shorting': True, 'Switched-On': False}
...
def Bitstamp_MOQ(Coin):
    ...
    MOQ = 5.00 / Bitstamp['Prices'][Coin] # MOQ is 5 USD / 5 EUR
...
def Get_MOQs(Coin, Exchange):
    ...
    if name == 'Kraken':
        Exchange.update({'MOQs': Kraken_MOQ(Coin)})
    elif name == 'Bitstamp':
        Exchange.update({'MOQs': Bitstamp_MOQ(Coin)})
    elif name == 'Gdax':
        Exchange.update({'MOQs': Gdax_MOQ(Coin)})
    elif name == 'Bl3p':
        Exchange.update({'MOQs': Bl3p_MOQ(Coin)})
    elif name == 'theRock':
        Exchange.update({'MOQs': theRock_MOQ(Coin)})
    elif name == 'Cex':
        Exchange.update({'MOQs': Cex_MOQ(Coin)})
    elif name == 'Wex':
        Exchange.update({'MOQs': Wex_MOQ(Coin)})
    elif name == 'BitBay':
        Exchange.update({'MOQs': BitBay_MOQ(Coin)})
    elif name == 'Quoinex':
        Exchange.update({'MOQs': Quoinex_MOQ(Coin)})
...
def Place_Limit_Order(Coin, Exchange, amount, side, price, Leverage):
    ...
    if Test_Mode:
        ...
        if Exchange['Name'] == 'Kraken':
            ...
            Wex_Open_Order = Wex_Limit_Order(Coin, amount, side, price, Leverage)
            Wex_Order_ID = str(Wex_Open_Order['id'])
            Open_Order = {'ID': Wex_Order_ID}
            
        elif Exchange['Name'] == 'BitBay':            
            BitBay_Open_Order = BitBay_Limit_Order(Coin, amount, side, price, Leverage)
    ...
    for attempt in range(1, attempts + 1): 
        ...
        try:
            ...
            if Exchange['Name'] == 'Kraken':
                ...
                return {'Order': 'Kraken_Dummy_Order', 'Placed': True}                
    ...
    while True:   
        ...
        if Sell_Order['Placed'] is False:
            ...
            if Filled is False:
                ...
                print 'Coin: ' + str(Coin) + \
                      str(Exchange1['Name']) + ' Buy Order ID: '+ \
        ...
        if Filled is False:
            ...
            if Filled is False:
                ...
                print 'Coin: ' + str(Coin) + \
                      str(Exchange2['Name']) + ' Sell Order ID: '+ \
        ...
        if Filled is False:
            ...
            Abort_Order = Execute_Market_Order(Coin, Exchange1, Buy_Amount, 'sell')
