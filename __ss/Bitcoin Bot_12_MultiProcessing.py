...
def thread_of_threads(Exchange):
    pool = ThreadPool(processes=len(Coins))
    Prices = {}
    for Coin in Coins:
        try:
            async_result = pool.apply_async(Get_Prices, (Exchange, Coin))
            print Prices
        except:
            pass
        Prices.update(async_result.get())
...
for Exchange in Exchanges:
    pool = ThreadPool(processes=len(Exchanges))
    try:
        async_result = pool.apply_async(thread_of_threads, (Exchange,))

    except:
        pass
    
    Exchange.update({'Prices': async_result.get()})
