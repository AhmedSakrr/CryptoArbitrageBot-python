...
@threaded # decorated with the threaded function
def thread_of_threads(Exchange):
    Prices = {}
    for Coin in Coins:
        try:
            x = Get_Prices(Exchange, Coin)
            result = x.result_queue.get()
            print Prices
        except:
            pass
...
for Exchange in Exchanges:
    try:
        x = thread_of_threads(Exchange)
        result = x.result_queue.get()
    except:
        pass
    
    Exchange.update({'Prices': result})
