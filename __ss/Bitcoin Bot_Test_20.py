    ...
    for attempt in range(1, Attempts):
        try:
            if exchange['Name'] == 'Kraken':
                ...
                Balance = Balances['Kraken']['EUR']
                Buy_Amount = liquid_factor*(Balance / Exchange_Basic['buy'])
                Liquid_Price_Buy = Liquidity_Check(exchange, Buy_Amount, 'buy')
                Sell_Amount = liquid_factor*(Balance / Exchange_Basic['sell'])
                Liquid_Price_Sell = Liquidity_Check(exchange, Sell_Amount, 'sell')
                
                   
            elif exchange['Name'] == 'Bitstamp':
                Exchange_Basic = Bitstamp_Basic() 
                Balance = Balances['Bitstamp']['EUR']               
                Buy_Amount = liquid_factor*(Balance / Exchange_Basic['buy'])
                Liquid_Price_Buy = Liquidity_Check(exchange, Buy_Amount, 'buy')
                Sell_Amount = liquid_factor*(Balance / Exchange_Basic['sell'])
                Liquid_Price_Sell = Liquidity_Check(exchange, Sell_Amount, 'sell') 
                ...
                Balance = Balances['theRock']['EUR']
                Buy_Amount = liquid_factor*(Balance / Exchange_Basic['buy'])
                Liquid_Price_Buy = Liquidity_Check(exchange, Buy_Amount, 'buy')
                Sell_Amount = liquid_factor*(Balance / Exchange_Basic['sell'])
                Liquid_Price_Sell = Liquidity_Check(exchange, Sell_Amount, 'sell')
                
            Exchange_Basic.update({'entry_buy': round(Liquid_Price_Buy, 2),
                                   'entry_sell': round(Liquid_Price_Sell, 2)})                                       
            
#            elif exchange['Name'] == 'Gatecoin':
#                Exchange_Basic = Gatecoin_Basic()           
        
        except:
            print 'Failed to get Enter Prices for %s on Attempt: %i' %(name, 
                                                                     attempt)
            log.write('\nFailed to Enter Prices for %s on Attempt: %i'%(name, 
                                                                      attempt))
            time.sleep(3)
            continue
        ...
        print 'Failed to get Enter Prices for %s ' %name
        log.write('\nFailed to Enter Prices for %s ' %name)
        return Blank
        
def Get_Exit_Prices(exchange):
    ...
    for attempt in range(1, Attempts):
        try:
            if exchange['Name'] == 'Kraken':
                Exchange_Basic = Kraken_Basic()
                Balance = Balances['Kraken']['BTC']
                Buy_Amount = liquid_factor * -Balance
                Liquid_Price_Buy = Liquidity_Check(exchange, Buy_Amount, 'buy')
                Sell_Amount = liquid_factor * Balance
                Liquid_Price_Sell = Liquidity_Check(exchange, Sell_Amount, 'sell')
                
        
            elif exchange['Name'] == 'Bitstamp':
                Exchange_Basic = Bitstamp_Basic()
                Balance = Balances['Bitstamp']['BTC']
                Buy_Amount = liquid_factor * -Balance
                Liquid_Price_Buy = Liquidity_Check(exchange, Buy_Amount, 'buy')
                Sell_Amount = liquid_factor * Balance
                Liquid_Price_Sell = Liquidity_Check(exchange, Sell_Amount, 'sell')
...
#                Exchange_Basic = Gatecoin_Basic() 
            ...
            Exchange_Basic.update({'exit_buy': round(Liquid_Price_Buy, 2),
                                   'exit_sell': round(Liquid_Price_Sell, 2)})
                                       
            
          
        
        except:
            print 'Failed to get Exit Prices for %s on Attempt: %i' %(name, 
                                                                     attempt)
            log.write('\nFailed to Exit Prices for %s on Attempt: %i'%(name, 
                                                                      attempt))
            time.sleep(3)
            continue
        ...
        print 'Failed to get Exit Prices for %s ' %name
        log.write('\nFailed to Exit Prices Info for %s ' %name)
        return Blank        
...
def Update(Arb):
    ...
    Arb[0].update(Get_Exit_Prices(Arb[0]))
    Arb[1].update(Get_Exit_Prices(Arb[1]))
...
def Find_Opportunities(perm):
    ...
    Enter_Target = Target_Profit/2 + Fees/2
    Exit_Target = -100.0
...
def Print_Main_Info():
    ...
    print '%-22s' '%-15s' '%-15s' % \
          ('Exchanges', ' BTC Balance', ' EUR Balance')
    ...
    print '%-22s' '%-12s' '%-12s' '%-12s' '%-12s' '%-12s' '%-12s' % \
          ('Exchanges', '  Ask', '  Bid', 'Entry Ask', 'Entry Bid',  'Exit Ask', 'Exit Bid')
    ...
    while True:   
        ...
        if Sell_Order['Placed'] is False:
            ...
            if Filled is False:
                ...
                Abort_Price = Get_Exit_Prices(Exchange1)['sell'] 
        ...
        if Filled is False:
            ...
            if Filled is False:
                ...
                Abort_Price = Get_Exit_Prices(Exchange2)['buy'] 
...
def Open(Opportunity):
    ...
    while True:
        ...
        Exit_Target = Spread -Target_Profit - Fees
...
for Exchange in Exchanges:
    Exchange.update(Get_Entry_Prices(Exchange))
    time.sleep(3)
    Exchange.update(Get_Exit_Prices(Exchange))
