...
class BaseClient(object):
    ...
    def _get(self, *args, **kwargs):
        ...
        Make a GET request.
        """
        return self._request(requests.get, t_o=5, *args, **kwargs)
    ...
    def _post(self, *args, **kwargs):
        ...
        kwargs['data'] = data
        return self._request(requests.post, t_o=30, *args, **kwargs)
    ...
    def _request(self, func, url, t_o=5, version=1, *args, **kwargs):
        """
        ...
        response = func(url, timeout=t_o, *args, **kwargs)
...
#        self._nonce = max(int(time.time()), nonce) # removed by GMB
    ...
    def limit_order(self, amount, side, price, base="btc", quote="usd"):
        """
        Order to buy amount of bitcoins for specified price.
        """
        data = {'amount': amount, 'price': price}
        url = self._construct_url(side+"/", base, quote)
        return self._post(url, data=data, return_json=True, version=2)
    
    def market_order(self, amount, side, base="btc", quote="usd"):
        """
        Order to buy amount of bitcoins for specified price.
        """
        data = {'amount': amount}
        url = self._construct_url(side+"/market/", base, quote)
        return self._post(url, data=data, return_json=True, version=2)
